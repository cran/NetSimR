---
title: "Sliced LogNormal-Pareto and Gamma-Pareto distributions"
author: "Yiannis Parizas"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Sliced LogNormal-Pareto and Gamma-Pareto distributions}
...
This vignette will cover the sliced distributions available in the NetSimR package. It will overview the theory behind these functions, propose possible uses and illustrate examples. Currently, Sliced distributions available are LogNormal and Gamma with a Pareto distribution for the tail.


## 2.Background theory


### 2.1 Sliced distribution background
...
Tail losses may be very influential on the premium, as such modelling the tail more accurately would significantly improve the accuracy of the premium. This becomes more important when modelling upper layers that will be mainly based on the severity tail.
...
Let claim severity $x$ have a probability density function $f(x)$ and a cumulative density function $F(x)$. Let $s$ be the slice point.
We define $f(x)$ as:
...
\end{array}\right]
$$
$g(x)$ is the attritional probability density function, fitted to the full claims. As suggested this could be fitted to full claims as a censored distribution or to the claims below $s$ as a truncated distribution for a better fit. $g(x)$ in NetSimR package will either be Gamma or LogNormal.
$h(x)$ is the large probability density function. $h(x)$ in NetSimR package will be a single parameter Pareto distribution. The scale parameter $x_{m}$ will be the slice point $s$. The shape parameter will be estimated from the claims larger than the slice point.
...
NetSimR package provides functions for the mean, capped mean, exposure curve and increased limit factor curves. These were discussed in the "Capped means, exposure curves and increased limit factors" vignette and will not be repeated here. The package also provides the probability density function (pdf), the cumulative probability density function (cdf) and the inverse cumulative probability function for the sliced distributions.
...
The cumulative density function provides probability of claims being less or more than a particular level. The distribution can be used alongside methods such as the Panjer recursion, in order to combine it with the frequency distribution and provide the aggregate claims distribution.

The inverse cumulative probability distribution can be used to run simulations over the defined distribution. Reinsurance structures can then be applied to use the simulation exercise for reinsurance pricing.
...
Here we calculate the expected severity from a sliced LogNormal-Pareto distribution, sliced at 10,000 currency units, with a Pareto shape parameter $shape = 1.2$ and lognormal parameters $mu = 6$ and $sigma = 1.6$.
```{r sliced mean}
library("NetSimR")
