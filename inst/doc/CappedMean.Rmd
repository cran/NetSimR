---
title: "Capped means, exposure curves and increased limit factors"
author: "Yiannis Parizas"
date: '`r format(Sys.Date(), "%d-%m-%Y")`'
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Capped means, exposure curves and increased limit factors}
...
This vignette will cover capped mean functions, exposure and increased limit factor curves of the NetSimR package. It will overview the theory behind these functions, propose possible uses and illustrate examples. The distributions covered for claim severities are LogNormal, Gamma, Pareto, Sliced LogNormal-Pareto, Sliced Gamma-Pareto. An application of the capped mean is suggested in the article "Taken to Excess" accessible [here](https://www.theactuary.com/features/2019/03/2019/03/06/taken-excess).
...
Let the claim capped at $c_{h}$ be $y_{h}$ and the claim capped at $c_{l}$ be $y_{l}$.
...
- Pricing/allocating premium of excess of loss layers.
...
### 4.1 Capped mean
The below code illustrates an alternative and more accurate approach to applying capping in regressions, for modelling attritional claims part.
The code steps are:
...
```{r capped mean example, eval=requireNamespace("crch", quietly = TRUE)}
library("NetSimR")
...
```
This would be interpreted as: the expected cost of the claims up to 1,500 currency units is 1.155 times the expected cost of the claims up to 1,000 currency units.
Other distributions follow the same logic.
