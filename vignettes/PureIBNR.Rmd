---
title: "Pure IBNR exposure from a reporting delay distribution"
author: "Yiannis Parizas"
date: '`r format(Sys.Date(), "%d-%m-%Y")`'
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Pure IBNR exposure from a reporting delay distribution}
...
Reporting delay is the time difference between when the accident occurred and when the insurer was notified.
Reporting delay is a positive and positively skewed variable.
It can be modelled with a LogNormal or Gamma regression.
...
Alternatively multiplying this with the premium and a loss ratio proxy will provide an acceptable approximation of the reserve.
The proportion of the unearned exposure multiplied by the premium will provide the unearned premium reserve.
One can use the exposure net of unearned period and pure IBNR as the offset (exposure) for modelling frequency in exposure or experience rating. This will be the modeller's choice rather than removing the last periods in situations where data is less.
...
Reinsurance structures are easier to implement on top of this method compared to triangles.
...
In the below example we will follow these steps:

- Construct a policy dataset with policy inception/expiry dates
- Generate claims
- Generate accident and reporting dates, through a reporting delay distribution
...
```{r pure IBNR example continued, eval=requireNamespace("crch", quietly = TRUE)}
#Generate dates data
PoliciesPerDay<-100
PeriodLength<-365*2
